...
**ForexFactory 403s GitHub-runner IPs, so the scrape goes through a reader proxy.** Measured from a runner (Azure egress): the calendar page returns 403 with the plain headers, with a full browser header set (`sec-ch-ua`, `Sec-Fetch-*`, `Upgrade-Insecure-Requests`), *and* with cookies fetched from the homepage first plus a `Referer`. It's an IP block, not a UA filter — don't waste another round trip trying to forge headers. The same request succeeds from a residential connection. The workflow therefore tries direct first (~0.1 s to the 403, and it self-heals if FF ever lifts the block) and then `https://r.jina.ai/<url>` with `x-respond-with: html`, which fetches from its own IPs and returns the raw page. The official `nfs.faireconomy.media` feed is *not* affected and needs no proxy.

Rules that matter if you touch this: the official feed wins inside its own time range (cache and scrape only contribute events *after* its last one — this is also what prevents duplicate rows if FF words a title differently on the page than in the feed); everything before the official feed's first event is pruned; and the output must stay **deterministic** — no timestamp goes in the file, or the 30-min cron would commit and rebuild Pages every single run. A failing scrape is explicitly non-fatal (`::warning::`, degrades to the old one-week behaviour) — check the workflow log for "Folgewoche nicht abrufbar". The builder also logs the scraped date range, because a proxy that swallowed the `?week=next` query would silently return the *current* week instead.
